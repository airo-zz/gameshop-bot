...
interface InputFieldForm {
  key: string
  label: string
  type: 'text' | 'number' | 'select' | 'quantity'
  placeholder: string
  required: boolean
  options: string[]
  min: string   // для type=quantity — минимально допустимое количество
  unit: string  // для type=quantity — единица (напр. «звёзд»)
}
...
const FIELD_TYPE_OPTIONS: { value: InputFieldForm['type']; label: string }[] = [
  ...
  { value: 'quantity', label: 'Количество (цена за единицу)' },
]
...
function normalizeInputField(raw: unknown): InputFieldForm {
  ...
  const type =
    f.type === 'number' || f.type === 'select' || f.type === 'quantity'
      ? f.type
      : 'text'
  return {
...
function serializeInputFields(fields: InputFieldForm[]): Record<string, unknown>[] {
  ...
  fields.forEach((f, idx) => {
    ...
    if (f.type === 'quantity') {
      out.min = Math.max(1, Math.floor(Number(f.min) || 1))
...
export default function ProductEditPage(props: ProductEditPageProps = {}) {
  ...
  function addInputField() {
    setForm((prev) => ({
      ...
      input_fields: [
        ...prev.input_fields,
        { key: '', label: '', type: 'text', placeholder: '', required: true, options: [], min: '', unit: '' },
      ],
    }))
  }
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      {/* Section: Поля товара (в основном — «Количество» для Stars/переменной цены).
          Обычные поля покупателя (логин/username) настраиваются на уровне игры/подраздела. */}
      <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-4 space-y-3">
        <h2 className="text-xs font-semibold text-white/50 uppercase tracking-wider">Поля товара</h2>
        <p className="text-xs text-white/40">
          Тип «Количество» — покупатель вводит число (напр. звёзд ≥ минимума), цена берётся
          за 1 единицу, итог = цена × количество. Для автовыдачи Stars задайте это поле здесь,
          а @username — в полях игры/подраздела.
        </p>
          ...
          <div key={idx} className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-3 space-y-2.5">
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder="Название поля (напр. Количество звёзд)"
                className={inputCls + ' flex-1'}
              />
              <button
                type="button" onClick={() => removeInputField(idx)} aria-label="Удалить поле"
            ...
            <div className="flex gap-2">
              <select
                value={field.type}
                onChange={(e) => updateInputField(idx, { type: e.target.value as InputFieldForm['type'] })}
                className={inputCls + ' flex-1'}
              >
            ...
            {field.type === 'select' ? (
              <textarea
                ...
                onChange={(e) => updateInputField(idx, { options: e.target.value.split('\n') })}
                rows={3} placeholder="Варианты — по одному на строку"
                className={inputCls + ' resize-none'}
              />
            ) : field.type === 'quantity' ? (
              <div className="flex gap-2">
                <input
                  type="number" min={1} value={field.min}
                  onChange={(e) => updateInputField(idx, { min: e.target.value })}
                  placeholder="Минимум (напр. 50)" className={inputCls + ' flex-1'}
                />
                <input
                  value={field.unit}
                  onChange={(e) => updateInputField(idx, { unit: e.target.value })}
                  placeholder="Единица (напр. звёзд)" className={inputCls + ' flex-1'}
                />
              </div>
            ) : (
              <input
                ...
                placeholder="Подсказка" className={inputCls}
              />
            )}
          </div>
        ))}
        ...
        <button
          type="button" onClick={addInputField}
