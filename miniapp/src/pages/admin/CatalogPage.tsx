...
import type { PaginatedResponse } from '@/api/admin'
import type { Product, Game } from '@/api'
import { useAdminCatalogStore } from '@/store/adminStore'
import { normalizeImageUrl } from '@/utils/imageUrl'

export default function CatalogPage() {
  const { page, gameSlug, search, setPage, setGameSlug, setSearch } = useAdminCatalogStore()
        ...
        <div className="space-y-2">
          {data.items.map((product, i) => (
            ...
            >
              {product.images[0] ? (
                <img
                  src={normalizeImageUrl(product.images[0]) ?? undefined}
                  alt={product.name}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-white/40">
                  {product.game_name ?? '—'} · {product.delivery_type}
                  {product.stock !== null && ` · склад: ${product.stock}`}
                </div>
              </div>
              <div className="text-sm font-semibold text-white shrink-0">
                {new Intl.NumberFormat('ru-RU', { style: 'currency', currency: product.currency, maximumFractionDigits: 0 }).format(product.price)}
              </div>
            </motion.div>
          ))}
        </div>
