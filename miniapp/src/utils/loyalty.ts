 ...
 *
 * NOTE: Реальные данные уровней (цвет, скидки, кэшбэк) приходят из API
 * через поля `profile.loyalty_color_hex` и `profile.loyalty_levels`.
 * Константы ниже используются только как статический фоллбэк/подсказка
 * на CheckoutPage, где профиль может быть недоступен.
 * ─────────────────────────────────────────────────────────────────────────
 */
...
/** Упрощённые данные для CheckoutPage (подсказка о прогрессе). Не использовать для цветов — брать из API. */
