...
ENUM_TYPES = {
    "delivery_type_enum": ("auto", "manual", "mixed"),
    "discount_type_enum": (
        "product",
        "category",
        "loyalty",
    ...
    "discount_value_type_enum": ("percent", "fixed"),
    "order_status_enum": (
        "new",
        "pending_payment",
        "paid",
    ...
    "payment_method_enum": ("balance", "card_yukassa", "usdt", "ton", "manual"),
    "payment_status_enum": ("pending", "succeeded", "failed", "refunded", "cancelled"),
    "admin_role_enum": ("owner", "admin", "manager", "operator", "content"),
    "ticket_status_enum": ("open", "in_progress", "waiting_user", "resolved", "closed"),
}


def enum(name: str) -> postgresql.ENUM:
    """Возвращает ENUM тип без автосоздания — типы создаём вручную через CREATE TYPE IF NOT EXISTS."""
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "cashback_percent", sa.Numeric(5, 2), nullable=False, server_default="0"
        ),
        ...
        sa.Column(
            ...
            sa.String(7),
            nullable=False,
            server_default=sa.text("'#CD7F32'"),
        ),
        sa.Column(
            "icon_emoji", sa.String(8), nullable=False, server_default=sa.text("'🥉'")
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "first_name", sa.String(128), nullable=False, server_default=sa.text("''")
        ),
        ...
        sa.ForeignKeyConstraint(
            ["loyalty_level_id"], ["loyalty_levels.id"], ondelete="SET NULL"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "currency", sa.String(8), nullable=False, server_default=sa.text("'RUB'")
        ),
    ...
    op.create_index(
        "ix_product_keys_available", "product_keys", ["product_id", "is_used"]
    )
    ...
    op.create_table(
        ...
        sa.Column("name", sa.String(128), nullable=False),
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column("type", enum("discount_type_enum"), nullable=False),
        sa.Column("target_id", postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column(
            "discount_value_type",
            enum("discount_value_type_enum"),
            nullable=False,
            server_default=sa.text("'percent'"),
        ),
        sa.Column("discount_value", sa.Numeric(10, 2), nullable=False),
        sa.Column(
            "min_order_amount", sa.Numeric(12, 2), nullable=False, server_default="0"
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["discount_rule_id"], ["discount_rules.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_index(
        "ix_promo_code_usages_promo_code_id", "promo_code_usages", ["promo_code_id"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["user_id"], ["users.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["promo_code_id"], ["promo_codes.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_table(
        ...
        sa.Column("user_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column(
            "cart_snapshot", postgresql.JSONB(astext_type=sa.Text()), nullable=False
        ),
        sa.Column(
        ...
        sa.Column(
            "second_reminder_sent", sa.Boolean(), nullable=False, server_default="false"
        ),
    ...
    op.create_table(
        ...
        sa.Column("payment_method", enum("payment_method_enum"), nullable=True),
        sa.Column("promo_code_id", postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column("paid_at", sa.DateTime(timezone=True), nullable=True),
    ...
    # FK от product_keys к order_items (после создания обеих таблиц)
    ...
    op.create_table(
        ...
        sa.Column("order_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("from_status", enum("order_status_enum"), nullable=True),
        sa.Column("to_status", enum("order_status_enum"), nullable=False),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["order_id"], ["orders.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["discount_rule_id"], ["discount_rules.id"], ondelete="RESTRICT"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "ix_order_discount_log_order_id", "order_discount_log", ["order_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column("order_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("user_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("method", enum("payment_method_enum"), nullable=False),
        sa.Column(
            "status",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["added_by_id"], ["admin_users.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("telegram_id"),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["assigned_to_id"], ["admin_users.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "is_template_response", sa.Boolean(), nullable=False, server_default="false"
        ),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
...
def downgrade() -> None:
    ...
    op.drop_foreign_key("fk_product_keys_order_item", "product_keys")
