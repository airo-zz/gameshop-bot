...
class TicketMessage(Base, UUIDMixin):
    ...
    sender_type: Mapped[str] = mapped_column(
        String(8), nullable=False   # "user" или "admin"
    )
    sender_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    text: Mapped[str] = mapped_column(Text, nullable=False)
    attachments: Mapped[list[str]] = mapped_column(ARRAY(String), nullable=False, default=[])
    is_template_response: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
...
class SupportTemplate(Base, UUIDMixin):
    ...
    sort_order: Mapped[int] = mapped_column(mapped_column.__class__, nullable=False, default=0)
...
# Исправляем sort_order
from sqlalchemy import Integer
SupportTemplate.sort_order = mapped_column(Integer, nullable=False, default=0)  # type: ignore
...
DEFAULT_PERMISSIONS: dict[AdminRole, list[str]] = {
    ...
    AdminRole.admin: [
        ...
        "orders.*", "users.*", "discounts.*",
        "support.*", "analytics.view",
    ],
    AdminRole.manager: [
        "orders.*", "users.view", "users.edit",
        "discounts.*", "support.*", "analytics.view",
    ],
    AdminRole.operator: [
        "orders.view", "orders.update_status", "orders.add_notes",
        "support.*", "users.view",
    ],
    AdminRole.content: [
        "games.*", "categories.*", "products.*",
...
class AdminUser(Base, UUIDMixin, TimestampMixin):
    ...
    role: Mapped[AdminRole] = mapped_column(
        Enum(AdminRole, name="admin_role_enum"), nullable=False, default=AdminRole.operator
    )
...
class AdminActionLog(Base, UUIDMixin):
    ...
    admin: Mapped[AdminUser] = relationship(
        "AdminUser", back_populates="action_logs",
